...
const ConstantsPlugin = (app) => {
	...
	const ENV_STATIC = {
		...
		UNI_PLATFORM: process.env.UNI_PLATFORM, // process 已经在env.d.ts声明类型 这里直接引用
	}
	...
	uni.$unc.$constants = $constants;	//全平台支持
	console.log(`成功配置全局常量插件,当前编译平台=${uni.$unc.$constants.UNI_PLATFORM}, 环境变量=${import.meta.env.VITE_STATIC_HOST}`);
}
