...
export default [
    ...
    },
    {
        ...
        method: 'get',
        /**
         * @param query 包含请求的查询参数。
         * @param body  包含请求的主体数据，通常用于 POST 请求。
         * @param headers   包含请求的头部信息。
         * @param params    包含请求的路径参数。
         */
        timeout: 500, //设置模拟请求的时间
        response: ({ query, body, headers, params }) => {
            //模拟后台数据处理 是不支持 前端uni挂载的变量
            //console.log(query, params, "=========") // 将会在控制台上打印语句 mock 模拟的是在服务器中进行的而不是在
            //自定义模拟响应分页数据
            const { page = 1, size = 10 } = query
            const res = _queryList({pageNo: page, pageSize: size}, 24)
            return {
                code: 0,
                message: 'ok',
                data: res.data
