...
function isIdcard(maybeIdcard){
	...
	if (!reg.test(maybeIdcard)) {//判断是否全为18或15位数字，最后一位可以是大小写字母X
		return false;
	}
	if (maybeIdcard.length == 18) {
		return CheckIdCard.province(maybeIdcard)&&CheckIdCard.brithday18(maybeIdcard)&&CheckIdCard.validate(maybeIdcard) ;
	}
